	...
	public int[] update(Camera camera, int[] pixels) {
	  ...
	  for(int x=0; x<width; x=x+1) {
          ...
          drawEnd = height - 1;
		    ...
		    int texX = (int)(wallX * (textures.get(texNum).SZ));
		    if(side == 0 && rayDirX > 0) {
          texX = textures.get(texNum).SZ - texX - 1;
        }
		    if(side == 1 && rayDirY < 0) {
          texX = textures.get(texNum).SZ - texX - 1;
        }
		    ...
		    for(int y=drawStart; y<drawEnd; y++) {
		    	...
		    	int color;
		    	if(side==0) {
            color = textures.get(texNum).pixels[texX + (texY * textures.get(texNum).SZ)];
          }
          else {
            color = (textures.get(texNum).pixels[texX + (texY * textures.get(texNum).SZ)]>>1) & 8355711;//Make y sides darker
          }
